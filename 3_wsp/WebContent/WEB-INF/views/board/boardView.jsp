			...
			<div id="board-area">
               ...
               <% if(fList != null){ %>
				...
				<div>
					...
					<%-- <a href="delete.do?type=<%=type%>&no=<%=boardNo%>" class="btn btn-primary float-right">삭제</a>  --%>
					
					<!-- 삭제 버튼 클릭 시 해당 게시글 상태를 'N'으로 바꾸고 목록으로 돌아가기-->
					<button id="deleteBtn" class="btn btn-primary float-right">삭제</button> 
					
					<a href="updateForm.do?type=<%=type%>&cp=<%=cp%>&no=<%=boardNo%>" class="btn btn-primary float-right ml-1 mr-1">수정</a>
					<% } %>
	...
	<script>
		$("#deleteBtn").on("click", function(){
			if(confirm("정말 삭제 하시겠습니까?")){
				location.href="delete.do?type=<%=type%>&no=<%=boardNo%>";
			}
		});
	</script>
</body>
