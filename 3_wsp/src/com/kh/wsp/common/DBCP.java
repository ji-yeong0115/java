...
public class DBCP {
	 ...
	 * 	 매번 실행 해야하는 비효율적인 모습을 보여줬다.
	 * 
	 * - DBCP는 WAS(Tomcat, Servlet, Container)가 
	 * 	 DB와의 연결을 미리 여러 개 만들어 가지고 있다가 클라이언트의
	 *	 요청 시마다 만들어둔 연결을 가져가 사용하게 하고,
	 *	 사용이 완료되면 반환하게 하는 기술.
	 *	(Connection 객체를 만들어 웅덩이에 모아놓은 모양이라고 해서
	...
	public static Connection getConnection() throws NamingException, SQLException {
		...
		*/
		...
		Context envContext = (Context)initContext.
					lookupLink("java:/comp/env");
		
		DataSource ds = (DataSource)envContext.lookup("jdbc/oracle");
		// DataSource : DriverManager를 대체하는 객체로
		// Connection 생성, Connection Pool구현, 관리하는 객체
